...
#' @param n the probability of a "no-show"
#'
...
ntickets <- function(N, gamma, p) {
    ...
    plot(searchd, -1 * objd(gamma, N, searchd, p), xlim=c(N, nd + 20), main=paste("Objective Vs n to find optimal tickets sold\n(", nd, ") gamma = ", gamma, " N =", N, " discrete", sep=""), xlab="n", ylab="Objective")

    abline(h=0, col="red")
    abline(v=nd, col="red")

    ## Plot the continuous
    curve(-1 * objc(gamma, N, x, p), from = N, to = N + 20, main=paste("Objective Vs n to find optimal tickets sold\n(", nc, ") gamma = ", gamma, " N =", N, " continuous", sep=""), xlab = "n", ylab="Objective")
...
objd <- function(gamma, N, n, p) {
    return (pbinom(N, n, p) + gamma  - 1)
}

## Continuous objuective function
objc <- function(gamma, N, n, p) {
    return (pnorm(N + 0.5, n * p, sqrt(n * p * (1 - p))) + gamma - 1)
}
